...
>
	...
	<div class="flex justify-center w-full sm:w-auto">
		<div class="flex gap-4 text-3xl">
			...
			{#if settings.links.github}
				<a href={settings.links.github} class="i-mdi-github" />
			{/if}
			<!-- svelte-ignore a11y-missing-content -->
			{#if settings.links.linkedin}
				<a href={settings.links.linkedin} class="i-mdi-linkedin" />
			{/if}
			<!-- svelte-ignore a11y-missing-content -->
			{#if settings.links.instagram}
				<a href={settings.links.instagram} class="i-mdi-instagram" />
			{/if}
		</div>
	</div>
</footer>
